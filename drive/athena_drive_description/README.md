...
2. Install the package by calling `colcon build --symlink-install --packages-select athena_drive_description`
3. (Re-)Source environment `source install/setup.bash`


> **NOTE:** If you use [RosTeamWorkspace (RTW)](https://rtw.b-robotized.com) than instead of the previous three steps, use `cb athena_drive_description` command.

Now, launch description test:
```
ros2 launch athena_drive_description view_athena_drive.launch.xml
```
or
```
ros2 launch athena_drive_description view_athena_drive.launch.py
```
